...
- **Windows 10/11**（64 位元）
- **Python 3.13+**
- **uv**（套件管理工具）：[安裝 uv](https://docs.astral.sh/uv/getting-started/installation/)
- **鼎新會計傳票系統**：已安裝並能正常執行
- **Excel 檔案**：符合規定格式
...
3. **驗證安裝**
   ```bash
   uv run python -c "from pywinauto import Application; print('✓ 安裝成功')"
   ```
...
# 指定 Excel 檔案路徑
uv run python src/main.py --excel-path /path/to/your/file.xlsx
```
...
2. 執行打包
```bash
uv run pyinstaller VoucherAutomate.spec
```
